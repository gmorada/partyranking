...
server.get('/events', function (request, response) {
	...
	graph.get('/fql?q=SELECT pic_big%2C name%2C venue%2C location%2C start_time%2C eid FROM event WHERE eid IN (SELECT eid FROM event_member WHERE uid %3D me()) order by start_time ASC LIMIT 10&access_token=' + request.cookies['XSRF-TOKEN'], function (error, event) {
